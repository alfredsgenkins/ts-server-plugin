...
import { CLASS_PLUGIN_METHOD_TYPE, CLASS_PLUGIN_PROPERTY_TYPE, CLASS_PLUGIN_STATIC_TYPE, FUNCTION_PLUGIN_TYPE } from "./util/config";
...
const getExportDepth = (node: ts.Node, accDepth: number = 0): number => {
...
export const getCompletionStringsForNode = (
    ...
    const completionMap: Record<SupportedCompletionDepths, CompletionStringsGetter> = {
        [NS_DECLARATION]: () => {
            ...
            return Object.keys(cache.declarationMap);
        },
        [TYPE_DECLARATION]: () => {
    ...
    const completionStringsGetter: CompletionStringsGetter =
        completionMap[exportDepth as SupportedCompletionDepths] || (() => ([]));
